  ...
  try {
    ...
    try {
      ...
      if (pidIsRunning(existingPid)) {
        ...
        console.log(`[watchdog] opc-server already running under pid ${existingPid}. Exiting duplicate.`);
        return false;
      }
...
function startChild() {
  ...
  child.on("exit", (code, signal) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[watchdog] opc-server exited (${reason}). Restarting in ${Math.round(restartDelayMs / 1000)}s...`);
